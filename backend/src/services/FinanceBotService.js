...
class FinanceBotService {
  async handleChat(message, username, originalMessage = null) {
    // Use originalMessage for saving if provided, otherwise use message
    ...
    // 2. Save user message (use original language version if available)
    await financeChatRepository.saveConversation(
      username,
      "user",
      messageToSave
    );
    let userMessage = message; // processed message for AI
    try {
      ...
      if (assistantMsg?.tool_calls?.length) {
        for (const toolCall of assistantMsg.tool_calls) {
          ...
          if (toolName === "extractMetric" && toolResult?.message) {
            ...
            if (toolResult.value && toolResult.metricName) {
              ...
              const cleanMetricName = toolResult.metricName.replace(
                /<[^>]*>/g,
                ""
              );
              ...
              const dateRangeMatch =
                toolResult.message.match(/from (.+?) to (.+?),/);
