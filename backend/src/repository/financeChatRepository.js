...
class FinanceChatRepository extends BaseMongoRepository {
  constructor() {
    ...
    try {
      ...
      if (messages.length >= 2) {
        ...
        if (lastMsg.isFeedbackResponse && secondLastMsg.isFeedbackResponse && 
      ...
      return messages.map(({ role, content, feedback, isFeedbackResponse, timestamp, downloadUrl }) => ({ 
        role, 
        content,
        feedback,  // Include feedback in the returned data
        isFeedbackResponse, // Include flag for feedback response messages
        timestamp: timestamp ? timestamp.toISOString() : new Date().toISOString(), // Include timestamp for better tracking
  ...
  async saveConversation(username, role, content, isFeedbackResponse = false, downloadUrl = null) {
...
async saveFeedback(username, messageIndex, feedbackType) {
  try {
    ...
    console.log(`DB: Total messages: ${doc.chats.length}, Bot messages: ${assistantMessages.length}, Target index: ${messageIndex}`);
    
    // Validate the index
    if (messageIndex < 0 || messageIndex >= assistantMessages.length) {
      console.error(`Bot message index ${messageIndex} out of bounds (max: ${assistantMessages.length - 1})`);
      return false;
    }
    ...
    if (targetAssistantMsg._id) {
      actualIndex = doc.chats.findIndex(msg => 
        msg._id && targetAssistantMsg._id && 
    ...
    if (actualIndex === -1) {
      console.log("ID match failed, trying to match by message content and timestamp...");
      actualIndex = doc.chats.findIndex(msg =>
        msg.role === targetAssistantMsg.role &&
        msg.content === targetAssistantMsg.content &&
        !msg.isFeedbackResponse &&
        msg.timestamp && targetAssistantMsg.timestamp &&
    ...
    if (actualIndex === -1) {
      console.log("Content+timestamp match failed, trying just by content...");
      actualIndex = doc.chats.findIndex(msg =>
        msg.role === targetAssistantMsg.role &&
        msg.content === targetAssistantMsg.content &&
        !msg.isFeedbackResponse
    ...
    if (actualIndex === -1) {
      console.error(`Could not find bot message at index ${messageIndex} in the chat history`);
      return false;
    }
    
    console.log(`Found bot message at actual index ${actualIndex} for bot index ${messageIndex}`);
    
    // Check if this message already has feedback (to avoid duplicate processing)
    if (doc.chats[actualIndex].feedback === feedbackType) {
      console.log(`Message at index ${actualIndex} already has feedback '${feedbackType}', skipping update`);
      return true;
    }
      // Create the update query to set feedback for specific message
    ...
    console.log(`Successfully updated feedback for message at ${actualIndex} to '${feedbackType}'`);
    return true;
  } catch (error) {
    console.error(`Error saving feedback for message ${messageIndex}:`, error);
    return false;
