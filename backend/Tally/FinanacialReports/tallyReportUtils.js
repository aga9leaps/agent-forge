...
// Function to fetch a Tally report and upload the PDF to S3
export async function fetchTallyReportAndUploadPDF({
  ...
  try {
    console.log(`Fetching ${reportName} from Tally server...`);
    ...
    if (response && response.data) {
      console.log(`Successfully received ${reportName} data from Tally`);
      
      // Generate PDF from the HTML response
      ...
      // Parse the PDF content for structured data
...
export function findMatchingMetric(pdfData, query) {
  ...
  if (lowerQuery.includes('profit margin')) {
    console.log("Special handling for profit margin metric - should be coming from ratio analysis report");
    
    // First try net profit percentage (primary metric for profit margin)
    if (pdfData['net_profit_percentage'] || pdfData['net_profit_%']) {
      // Handle both formats of the key
      const netProfitPct = pdfData['net_profit_percentage'] || pdfData['net_profit_%'];
      let cleanedValue = cleanValue(netProfitPct);
    ...
    if (pdfData['gross_profit_percentage'] || pdfData['gross_profit_%']) {
      // Handle both formats of the key
      const grossProfitPct = pdfData['gross_profit_percentage'] || pdfData['gross_profit_%'];
      let cleanedValue = cleanValue(grossProfitPct);
...
export function detectReportTypeFromQuery(query) {
  ...
  // First check for profit margin which should always come from ratio analysis report
  if (lowerQuery.includes('profit margin')) {
    console.log("Detected profit margin query - directing to ratio analysis report");
    return 'ratio_analysis_report';
  }
  
  // Next, check if specific P&L metrics like "gross loss b/f" are mentioned
  ...
  for (const [userTerm, technicalTerms] of Object.entries(TERM_MAPPING)) {
    if (lowerQuery.includes(userTerm)) {
      ...
      console.log(`Defaulting to profit_and_loss_report for metric: ${userTerm}`);
      return 'profit_and_loss_report';
    }
  }
  
  // If all else fails, default to profit & loss
  console.log("Could not detect report type, defaulting to profit_and_loss_report");
  return 'profit_and_loss_report';
}